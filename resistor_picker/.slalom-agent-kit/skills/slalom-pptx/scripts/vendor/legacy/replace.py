...
VENDORED COPY — slalom-pptx skill
Source: anthropic-agent-skills @ 69c0b1a06741, skills/pptx/scripts/replace.py
In-house patches:
  #7  Shape clearing is now gated on shape membership in the replacement
      JSON. Shapes OMITTED from the replacements inherit master text
      (matching slalom-pptx's documented Workflow A contract). Shapes
      explicitly present with no "paragraphs" key are still cleared
      (explicit blank). Pass {"paragraphs": []} to blank a shape; omit
      the shape key to keep its master text.
  #8  apply_paragraph_properties splits the run text on \x0b (vertical
      tab, python-pptx's representation of <a:br/>) and emits alternating
      runs and <a:br/> elements under the same <a:p>. Without this, the
      VT char round-trips through OOXML serialization as the literal
      escape "_x000B_" and PowerPoint/LibreOffice render that string
      visibly. Font properties are applied to every run, not just the
      first.
  #10 SLIDE_NUMBER placeholders that are absent from the replacement JSON
      are auto-filled with the slide's actual deck position (1-based).
      Workflow A documents that body slides MUST override SLIDE_NUMBER
      because the master ships hardcoded numbers; with #9 exposing those
      placeholders in inventory and #7 leaving omitted shapes alone, an
      auto-fill default aligns the tool with the documented contract.
      Explicit overrides in the replacement JSON still take precedence.
  #11 Overflow error messages now include placeholder_type and a 30-char
      preview of the master's first paragraph text, so failures point at
      a recognizable element instead of just "slide-N/shape-M".
"""
...
def apply_paragraph_properties(paragraph, para_data: Dict[str, Any]):
    ...
    # PATCH #8: split on \x0b (vertical tab, python-pptx's representation of
    # <a:br/>) and emit alternating runs + <a:br/> elements so soft line
    # breaks survive OOXML serialization. Without this split the VT char
    # serializes as the literal escape "_x000B_" and renders visibly.
    segments = text.split("\x0b")

    # First segment lands in the leading run (existing or freshly added).
    ...
    # Subsequent segments: append <a:br/> then a new run, in document order.
    p_elem = paragraph._p
    ...
    # Apply font properties to every run so formatting carries across breaks.
    for run in runs:
...
def apply_replacements(pptx_file: str, json_file: str, output_file: str):
    ...
    for slide_key, shapes_dict in inventory.items():
        ...
        # PATCH #7: only mutate shapes the caller actually included in their
        # replacement JSON. Shapes omitted from `replacements[slide_key]`
        # inherit their master text. Explicit blanking still works by passing
        # an entry with `"paragraphs": []`.
        slide_replacements = replacements.get(slide_key, {})

        # Process each shape from inventory
        for shape_key, shape_data in shapes_dict.items():
            shapes_processed += 1

            if shape_key in slide_replacements:
                replacement_shape_data = slide_replacements[shape_key]
            elif shape_data.placeholder_type == "SLIDE_NUMBER":
                # PATCH #10: SLIDE_NUMBER placeholders ship with hardcoded
                # master values (e.g., "29", "94") that Workflow A documents
                # MUST be overridden on every body slide. Treat omission as
                # "fill with the slide's actual deck position" rather than
                # "inherit master" (the master value is always wrong on
                # extracted slides). Explicit overrides still take precedence
                # via the branch above.
                replacement_shape_data = {
                    "paragraphs": [{"text": str(slide_index + 1)}]
                }
            else:
                continue  # omitted -> keep master text intact
    ...
    for slide_key, shape_overflows in updated_overflow.items():
        for shape_key, new_overflow in shape_overflows.items():
            ...
            if new_overflow > original + 0.01:  # Small tolerance for rounding
                ...
                # PATCH #11: Surface placeholder_type and a 30-char preview of
                # the master text so the error names the recognizable element
                # ("CUSTOMER STORY: INDUSTRY...") rather than just shape-N.
                shape_descriptor = f"{slide_key}/{shape_key}"
                shape_data = updated_inventory.get(slide_key, {}).get(shape_key)
                if shape_data is not None:
                    parts = []
                    if shape_data.placeholder_type:
                        parts.append(shape_data.placeholder_type)
                    paragraphs = shape_data.paragraphs
                    if paragraphs and paragraphs[0].text:
                        preview = paragraphs[0].text[:30]
                        if len(paragraphs[0].text) > 30:
                            preview += "..."
                        parts.append(f'"{preview}"')
                    if parts:
                        shape_descriptor += f" ({', '.join(parts)})"

                overflow_errors.append(
                    f'{shape_descriptor}: overflow worsened by {increase:.2f}" '

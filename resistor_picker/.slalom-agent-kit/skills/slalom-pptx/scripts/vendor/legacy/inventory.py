...
Source: anthropic-agent-skills @ 69c0b1a06741, skills/pptx/scripts/inventory.py
In-house patches:
  #6  _get_default_font_size now prefers the layout-level default already
      captured by the static get_default_font_size during __init__, so the
      overflow detector stops spuriously firing on layouts that override the
      master defRPr (e.g. device-frame-1's 5pt body).
  #9  is_valid_shape no longer filters out SLIDE_NUMBER placeholders. They
      ship from the master with hardcoded position text (e.g., "29", "94")
      that Workflow A documents MUST be overridden on every body slide;
      filtering them hid editable content and made the documented contract
      impossible to fulfil via the JSON. The numeric-only FOOTER filter
      below stays - those carry no editable content.
...
class ShapeData:
    ...
    def _get_default_font_size(self) -> int:
        """Get default font size from layout defRPr (preferred) or theme fallback.

        PATCH #6 (slalom-pptx): Prefer self.default_font_size, which is populated
        in __init__ by the static get_default_font_size() walking the LAYOUT's
        defRPr. The master-bodyStyle/titleStyle walk below stays as a fallback
        for cases where layout-level defaults aren't available. Without this
        patch, layouts that override the master defRPr (e.g. device-frame-1's
        body at 5pt) trigger spurious overflow errors because the detector
        falls back to the master's larger default (typically 18pt).
        """
        if self.default_font_size:
            return int(self.default_font_size)
        try:
            if not (
...
def is_valid_shape(shape: BaseShape) -> bool:
    ...
    # PATCH #9: SLIDE_NUMBER is editable per Workflow A's contract, so it
    # stays in the inventory. Numeric-only FOOTER placeholders still get
    # filtered - they carry no editable content (the master uses them as
    # decorative numeric stamps).
    if hasattr(shape, "is_placeholder") and shape.is_placeholder:  # type: ignore
        if shape.placeholder_format and shape.placeholder_format.type:  # type: ignore
            placeholder_type = (
                str(shape.placeholder_format.type).split(".")[-1].split(" ")[0]  # type: ignore

...
Source: anthropic-agent-skills @ 69c0b1a06741, skills/pptx/scripts/rearrange.py
In-house patches:
  #5  After shutil.copy2(...) chmod the output to 0o644 so prs.save() can't
      hit PermissionError when the source master ships read-only.
"""
...
def rearrange_presentation(template_path, output_path, slide_sequence):
    ...
    if template_path != output_path:
        ...
        # PATCH #5: shutil.copy2 preserves source mode; if the master ships
        # read-only (0o400), prs.save() below will raise PermissionError. Force
        # the output to 0o644 immediately so save can write back.
